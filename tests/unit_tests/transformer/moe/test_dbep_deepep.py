...
class MoEModelTestContainer:
    def __init__(
        ...
        **kwargs,
        ...
        _set_random_seed(seed_=123, data_parallel_random_init=True)
    ...
    def gen_expert_load(self, std: float):
        if torch.distributed.get_rank() == 0:
            # generate expert load, each load is in [0, 1], sum=1, std=std
            while True:
                expert_load = torch.randn((self.num_moe_experts,), dtype=torch.float32).cuda()
                expert_load = expert_load / expert_load.std() * std
                expert_load = expert_load + 1.0 - expert_load.sum() / self.num_moe_experts
                if expert_load.min() >= 0.0:
                    break
        else:
            expert_load = torch.zeros((self.num_moe_experts,), dtype=torch.float32).cuda()
        torch.distributed.broadcast(expert_load, src=0)
        if torch.distributed.get_rank() == 0:
            print(f"rank {torch.distributed.get_rank()} expert_load {expert_load}")
        return expert_load

    def enforce_expert_std(self, router: Router, hidden_states: torch.Tensor, expert_load: torch.Tensor, update_rate: float = 0.01):
        for iter in range(300):
            with torch.no_grad():
                _, routing_map = router(hidden_states)
                global_tokens_per_expert = routing_map.sum(dim=0).to(dtype=torch.float32)
                torch.distributed.all_reduce(
                    global_tokens_per_expert, op=torch.distributed.ReduceOp.SUM)
                avg_tokens = global_tokens_per_expert.sum() / self.num_moe_experts
                
                offset = expert_load - global_tokens_per_expert / avg_tokens
                updated_expert_bias = router.expert_bias + offset * update_rate
                # if torch.distributed.get_rank() == 0:
                #     print(f"rank {torch.distributed.get_rank()} iter {iter} global_tokens_per_expert {global_tokens_per_expert / avg_tokens}")
                #     print(f"rank {torch.distributed.get_rank()} iter {iter} expert_bias {router.expert_bias}")
                router.local_tokens_per_expert.zero_()
                router.expert_bias.copy_(updated_expert_bias)
        assert torch.allclose(
            global_tokens_per_expert / avg_tokens, expert_load, atol=0.01
        ), f"Global tokens per expert {global_tokens_per_expert / avg_tokens} does not match expert load {expert_load}"
    ...
    def perf_test(self):
        ...
        expert_load = self.gen_expert_load(std=self.std)
        self.enforce_expert_std(self.moe_layer.router, hidden_states, expert_load)
        self.enforce_expert_std(self.moe_layer_base.router, hidden_states, expert_load)
        ...
        time_buf = torch.zeros(4, dtype=torch.float32).cuda()
        ...
        if rank == 0:
            ...
            max_time = torch.zeros_like(time_buf, dtype=torch.float32).cuda()
            ...
            print(f"rank {rank} DBEP max output_splits: {max_time[2]}")
            print(f"rank {rank} Base max output_splits: {max_time[3]}")
    ...
    def dispatcher_perf_test(self):
        ...
        with self.model_base.no_sync():
            for i in range(warmup_iter):
                ...
                dispatched_input_base, tokens_per_expert_base, _ = self.token_dispatcher_base.token_permutation(hidden_states_base, probs_base, routing_map_base)
